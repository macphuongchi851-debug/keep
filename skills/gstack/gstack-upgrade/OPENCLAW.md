...
[ -z "$_AUTO" ] && _AUTO=$(/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
echo "AUTO_UPGRADE=$_AUTO"
```

**If `AUTO_UPGRADE=true` or `AUTO_UPGRADE=1`:** Skip ask the user directly in chat. Log "Auto-upgrading gstack v{old} → v{new}..." and proceed directly to Step 2. If `./setup` fails during auto-upgrade, restore from backup (`.bak` directory) and warn the user: "Auto-upgrade failed — restored previous version. Run `/gstack-upgrade` manually to retry."

**Otherwise**, use ask the user directly in chat:
...
```bash
/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME//home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME//home/keep/.openclaw/workspace/skills/gstack"
elif [ -d "/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR="/home/keep/.openclaw/workspace/skills/gstack"
elif [ -d "/home/keep/.openclaw/workspace/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR="/home/keep/.openclaw/workspace/skills/gstack"
elif [ -d "$HOME//home/keep/.openclaw/workspace/skills/gstack" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT//home/keep/.openclaw/workspace/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT//home/keep/.openclaw/workspace/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT//home/keep/.openclaw/workspace/skills/gstack"
  fi
fi
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/` when you're ready."
...
```bash
/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/bin/gstack-update-check --force 2>/dev/null || \
/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/` when you're ready."
