...
If the output is non-empty (working tree is dirty), **STOP** and use ask the user directly in chat:
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. ask the user directly in chat: "This site requires authentication. Want to import cookies from your browser? Run `/setup-browser-cookies` first if needed."
...
# exec(find) for file patterns matching the affected page
```
...
11. **Clean working tree required.** If dirty, use ask the user directly in chat to offer commit/stash/abort before proceeding.
