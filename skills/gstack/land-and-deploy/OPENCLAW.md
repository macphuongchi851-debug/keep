...
```bash
/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/bin/gstack-review-read 2>/dev/null
```
...
to guide deploy verification. If nothing is detected, ask the user via ask the user directly in chat
...
```bash
eval $(/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
4. If no deploy workflows detected and no URL provided: use ask the user directly in chat once:
...
If deploy fails (`conclusion` is `failure`): use ask the user directly in chat:
...
If any fail: show the evidence (screenshot path, console errors, perf numbers). Use ask the user directly in chat:
