...
**Enum & Value Completeness requires reading code OUTSIDE the diff.** When the diff introduces a new enum value, status, tier, or type constant, use exec(grep) to find all files that reference sibling values, then read those files to check if the new value is handled. This is the one category where within-diff review is insufficient.
...
Takes seconds, prevents recommending outdated patterns. If web_search is unavailable, note it and proceed with in-distribution knowledge.
...
If there are ASK items remaining, present them in ONE ask the user directly in chat:
...
If 3 or fewer ASK items, you may use individual ask the user directly in chat calls instead of batching.
...
2. **FALSE POSITIVE comments:** Present each one via ask the user directly in chat:
...
3. **VALID BUT ALREADY FIXED comments:** Reply using the **Already Fixed reply template** from greptile-triage.md — no ask the user directly in chat needed:
