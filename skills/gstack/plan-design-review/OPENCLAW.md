...
### 0D. Focus Areas
ask the user directly in chat: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. Want me to review all 7 dimensions, or focus on specific areas?"
...
5. ask the user directly in chat if there's a genuine design choice to resolve
...
**STOP.** ask the user directly in chat once per issue. Do NOT batch. Recommend + WHY. If no issues, say so and move on. Do NOT proceed until user responds.
...
**STOP.** ask the user directly in chat once per issue. Do NOT batch. Recommend + WHY.
...
```
  ...
  ...
```
Each decision = one ask the user directly in chat with recommendation + WHY + alternatives. edit the plan with each decision as it's made.

## CRITICAL RULE — How to ask questions
Follow the ask the user directly in chat format from the Preamble above. Additional rules for plan design reviews:
...
* **Escape hatch:** If a section has no issues, say so and move on. If a gap has an obvious fix, state what you'll add and move on — don't waste a question on it. Only use ask the user directly in chat when there is a genuine design choice with meaningful tradeoffs.
...
### TODOS.md updates
After all review passes are complete, present each potential TODO as its own individual ask the user directly in chat. Never batch TODOs — one per question. Never silently skip this step.
...
If any ask the user directly in chat goes unanswered, note it here. Never silently default to an option.
...
```bash
/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
