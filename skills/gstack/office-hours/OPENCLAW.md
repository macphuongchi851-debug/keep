...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT//home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/browse/dist/browse" ] && B="$_ROOT//home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/browse/dist/browse"
[ -z "$B" ] && B=/home/keep/.openclaw/workspace//home/keep/.openclaw/workspace/skills/gstack/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
3. Use exec(grep)/exec(find) to map the codebase areas most relevant to the user's request.
...
5. **Ask: what's your goal with this?** This is a real question, not a formality. The answer determines everything about how the session runs.
   ...
   Via ask the user directly in chat, ask:
...
Ask these questions **ONE AT A TIME** via ask the user directly in chat. Push on each one until the answer is specific, evidence-based, and uncomfortable. Comfort means the founder hasn't gone deep enough.
...
Ask these **ONE AT A TIME** via ask the user directly in chat. The goal is to brainstorm and sharpen the idea, not interrogate.
...
- Ask via ask the user directly in chat: "Should we build on this prior design or start fresh?"
...
**Privacy gate:** Before searching, use ask the user directly in chat: "I'd like to search for what the world thinks about this space to inform our discussion. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?"
Options: A) Yes, search away  B) Skip — keep this session private
If B: skip this phase entirely and proceed to Phase 3. Use only in-distribution knowledge.

When searching, use **generalized category terms** — never the user's specific product name, proprietary concept, or stealth idea. For example, search "task management app landscape" not "SuperTodo AI-powered task killer."

If web_search is unavailable, skip this phase and note: "Search unavailable — proceeding with in-distribution knowledge only."

**Startup mode:** web_search for:
- "[problem space] startup approach {current year}"
- "[problem space] common mistakes"
- "why [incumbent solution] fails" OR "why [incumbent solution] works"

**Builder mode:** web_search for:
...
Use ask the user directly in chat to confirm. If the user disagrees with a premise, revise understanding and loop back.
...
Present via ask the user directly in chat. Do NOT proceed without user approval of the approach.
...
Present the reviewed design doc to the user via ask the user directly in chat:
...
Then use ask the user directly in chat: "Would you consider applying to Y Combinator?"
...
- **Questions ONE AT A TIME.** Never batch multiple questions into one ask the user directly in chat.
